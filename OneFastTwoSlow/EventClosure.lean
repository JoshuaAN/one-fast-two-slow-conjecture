...
private theorem P_preserves_K2 (f : ValueFn) (hf : InCone f) :
  ...
  · simp [P]
    ...
    have h_lam : 0 ≤ f (x + 1 + 2, 0) + f (x + 1, 1)
                       - f (x + 1 + 1, 0) - f (x + 1 + 1, 1) := hf.K2 0 (x + 1)
    -- μ₁ term
    have h_bdry : 0 ≤ f (x + 1, 0) + f (x - 1, 1) - f (x, 0) - f (x, 1) := hatM 0
    -- μ₂ terms
    have h_k2 : 0 ≤ f (x + 2, 0) - 2 * f (x + 1, 0) + f (x, 0)         := convex_x f hf 0 x
    have h_K2 : 0 ≤ f (x + 2, 0) + f (x, 1) - f (x + 1, 0) - f (x + 1, 1) := hf.K2 0 x
    nlinarith [mul_nonneg hΛ_nn h_lam, mul_nonneg hμ₁_nn h_bdry,
               mul_nonneg hμ₂_nn h_k2, mul_nonneg hμ₂_nn h_K2]
  -- y = 1
  · simp [P]
    -- λ term
    have h_lam : 0 ≤ f (x + 1 + 2, 1) + f (x + 1, 2)
                       - f (x + 1 + 1, 1) - f (x + 1 + 1, 2) := hf.K2 1 (x + 1)
    -- μ₁ term
    have h_bdry : 0 ≤ f (x + 1, 1) + f (x - 1, 2) - f (x, 1) - f (x, 2) := hatM 1
    -- μ₂ terms
    have h_k2 : 0 ≤ f (x + 2, 1) - 2 * f (x + 1, 1) + f (x, 1)         := convex_x f hf 1 x
    have h_K2 : 0 ≤ f (x + 2, 0) + f (x, 1) - f (x + 1, 0) - f (x + 1, 1) := hf.K2 0 x
    nlinarith [mul_nonneg hΛ_nn h_lam, mul_nonneg hμ₁_nn h_bdry,
               mul_nonneg hμ₂_nn h_k2, mul_nonneg hμ₂_nn h_K2]
...
private theorem P_preserves_K3 (f : ValueFn) (hf : InCone f) :
  ...
  have hatS : ∀ z : Fin 2,
    ...
    · have hsub : x - 1 + 1 = x := Nat.sub_add_cancel hx
      ...
      rw [hsub] at h
      linarith
  -- Normalize (x+1) - 1 ↦ x in the goal (K3 doesn't touch (x+2) - 1).
  ...
  · show P p f (x, 0) + P p f (x + 1, 1)
    ...
    simp only [P]
    rw [e1]
    -- λ-term: K3 at x+1, row 0
    have h_lam  : 0 ≤ f (x + 1, 0) + f (x + 1 + 1, 1)
                       - f (x + 1 + 1, 0) - f (x + 1, 1) := hf.K3 0 (x + 1)
    -- μ₁-term: boundary Ŝ at x, row 0
    have h_bdry : 0 ≤ f (x - 1, 0) + f (x, 1) - f (x, 0) - f (x - 1, 1) := hatS 0
    -- μ₂-term: K3 at x, row 0
    have h_slow : 0 ≤ f (x, 0) + f (x + 1, 1) - f (x + 1, 0) - f (x, 1) := hf.K3 0 x
    nlinarith [mul_nonneg hΛ_nn h_lam, mul_nonneg hμ₁_nn h_bdry,
...
private theorem P_preserves_K4 (f : ValueFn) (hf : InCone f) :
  ...
  · show P p f (x, 1) - P p f (x, 0) ≥ 0
    simp only [P]
    -- λ-term
    have h_lam  : 0 ≤ f (x + 1, 1) - f (x + 1, 0) := hf.K4 0 (x + 1)
    -- μ₁-term
    have h_bdry : 0 ≤ f (x - 1, 1) - f (x - 1, 0) := hf.K4 0 (x - 1)
    -- μ₂-term
    have h_slow : 0 ≤ f (x, 1)     - f (x, 0)     := hf.K4 0 x
    nlinarith [mul_nonneg hΛ_nn h_lam, mul_nonneg hμ₁_nn h_bdry,
               mul_nonneg hμ₂_nn h_slow]
  -- y = 1
  · show P p f (x, 2) - P p f (x, 1) ≥ 0
    simp only [P]
    -- λ-term
    have h_lam  : 0 ≤ f (x + 1, 2) - f (x + 1, 1) := hf.K4 1 (x + 1)
    -- μ₁-term
    have h_bdry : 0 ≤ f (x - 1, 2) - f (x - 1, 1) := hf.K4 1 (x - 1)
    -- μ₂-term
    have h_slow : 0 ≤ f (x, 1)     - f (x, 0)     := hf.K4 0 x
    nlinarith [mul_nonneg hΛ_nn h_lam, mul_nonneg hμ₁_nn h_bdry,
               mul_nonneg hμ₂_nn h_slow]
...
private theorem P_preserves_K5 (f : ValueFn) (hf : InCone f) :
    ...
    have hΛ_nn  : 0 ≤ p.Λ  := p.hΛ_nn
    simp only [P]
    -- λ-term: K5 of f at x+1
    have h_lam  : 0 ≤ f (x + 1, 0) + f (x + 1, 2) - 2 * f (x + 1, 1) := hf.K5 (x + 1)
    -- μ₁-term: K5 of f at x-1 (truncated subtraction — K5 holds at every ℕ)
    have h_bdry : 0 ≤ f (x - 1, 0) + f (x - 1, 2) - 2 * f (x - 1, 1) := hf.K5 (x - 1)
...
private theorem P_preserves_K6 (f : ValueFn) (hf : InCone f) :
    ...
    rw [e1, e2]
    -- λ-term
    have h_lam  : 0 ≤ f (x + 1 + 2, 0) + f (x + 1, 2) - 2 * f (x + 1 + 1, 1) := hf.K6 (x + 1)
    -- μ₁-term
    have h_bdry : 0 ≤ f (x + 1, 0) + f (x - 1, 2) - 2 * f (x, 1) := hatR
    -- 2μ₂-term
    have h_slow : 0 ≤ f (x + 2, 0) + f (x, 1) - f (x + 1, 0) - f (x + 1, 1) := hf.K2 0 x
    nlinarith [mul_nonneg hΛ_nn h_lam, mul_nonneg hμ₁_nn h_bdry,
