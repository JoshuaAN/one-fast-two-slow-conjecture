...
theorem iterate_mem_cone (ρ : ℝ) (hρ : 0 < ρ) : ∀ m, InCone (iterate p ρ m) := by
  ...
  | succ k ih =>
    have hstep : iterate p ρ (k + 1) = valueStep p ρ (iterate p ρ k) := by
...
theorem relaxed_eq_true (f : ValueFn) (hb : BoundaryComp f) :
  ...
  · -- y = 0
    ...
    · show Q f (1, 0) = Q_true f (1, 0)
      simp only [Q, Q_true]; exact min_eq_left hb.b1
    · show Q f (2, 0) = Q_true f (2, 0)
      simp only [Q, Q_true]; rw [min_eq_left hb.b2]
  ...
  · -- y = 1
    ...
    · show Q f (1, 1) = Q_true f (1, 1)
      simp only [Q, Q_true]; exact min_eq_left hb.b2
