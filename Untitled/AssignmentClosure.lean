...
private theorem Q_preserves_K1 (f : ValueFn) (hf : InCone f) :
  ...
  -- K5 facts for f, with the Fin 2 → Fin 3 indices reduced to numerals.
  have K5_01 : ∀ (a : ℕ), f (a, 0) ≤ f (a, 1) := by
    intro a
    have h := hf.K5 0 a
  ...
  · match x with
    ...
    | 1 =>
      ...
      · apply le_min
        ...
        · exact le_trans (min_le_right _ _) (K5_12 0)
    | k + 2 =>
...
/-- (K2) Convexity in `x` at fixed `y` for `Q f`. Paper pages 8–9 table. -/
private theorem Q_preserves_K2 (f : ValueFn) (hf : InCone f) :
    ∀ (y : Fin 3) (x : ℕ), Q f (x + 2, y) - 2 * Q f (x + 1, y) + Q f (x, y) ≥ 0 := by
  intro y x
  -- Complete cone-fact menu for f (all seven families, plain numeral indices).
  have K1 : ∀ (z : Fin 3) (a : ℕ), f (a, z) ≤ f (a + 1, z) := by
    intro z a; have := hf.K1 z a; linarith
  have K2 : ∀ (z : Fin 3) (a : ℕ),
      f (a, z) + f (a + 2, z) - 2 * f (a + 1, z) ≥ 0 := by
    intro z a; have := hf.K2 z a; linarith
  have K3_0 : ∀ (a : ℕ),
  ...
  have K4_1 : ∀ (a : ℕ),
    ...
    rw [e1, e2] at h; linarith
  have K5_01 : ∀ (a : ℕ), f (a, 0) ≤ f (a, 1) := by
    intro a; have h := hf.K5 0 a
    have e0 : ((0 : Fin 2).castSucc : Fin 3) = 0 := rfl
    have e1 : ((0 : Fin 2).succ : Fin 3) = 1 := rfl
    rw [e0, e1] at h; linarith
  have K5_12 : ∀ (a : ℕ), f (a, 1) ≤ f (a, 2) := by
    intro a; have h := hf.K5 1 a
    have e1 : ((1 : Fin 2).castSucc : Fin 3) = 1 := rfl
    have e2 : ((1 : Fin 2).succ : Fin 3) = 2 := rfl
    rw [e1, e2] at h; linarith
  have K6 : ∀ (a : ℕ), f (a, 0) + f (a, 2) - 2 * f (a, 1) ≥ 0 := by
    intro a; have := hf.K6 a; linarith
  have K7 : ∀ (a : ℕ), f (a + 2, 0) + f (a, 2) - 2 * f (a + 1, 1) ≥ 0 := by
    intro a; have := hf.K7 a; linarith
  -- The full menu, instantiated below per case.
  fin_cases y
  -- ════════════════ y = 0 ════════════════
  · match x with
    | 0 =>
      show Q f (2, 0) - 2 * Q f (1, 0) + Q f (0, 0) ≥ 0
      have hQ2 := Q0_eq f 0          -- Q f (2,0) ∈ {f(2,0), f(1,1), f(0,2)}
      have hQ2' := Q0_le f 0
      have hQ1 := Q0_pos1_le f       -- Q f (1,0) ≤ f(1,0), ≤ f(0,1)
      have hQ0 : Q f (0, 0) = f (0, 0) := by simp only [Q]
      rcases hQ2 with e | e | e <;> rw [e, hQ0] <;>
      linarith [K1 0 0, K1 0 1, K1 1 0, K2 0 0, K2 1 0,
                K3_0 0, K4_0 0, K5_01 0, K5_12 0, K6 0, K7 0,
                hQ1.1, hQ1.2, hQ2'.1, hQ2'.2.1, hQ2'.2.2]
    | 1 =>
      show Q f (3, 0) - 2 * Q f (2, 0) + Q f (1, 0) ≥ 0
      have hQ3 := Q0_eq f 1
      have hQ3' := Q0_le f 1
      have hQ2 := Q0_le f 0          -- middle: upper bounds
      have hQ1 := Q0_pos1_eq f
      have hQ1' := Q0_pos1_le f
      rcases hQ3 with e3 | e3 | e3 <;> rcases hQ1 with e1 | e1 <;>
      rw [e3, e1] <;>
      linarith [K1 0 0, K1 0 1, K1 0 2, K1 1 0, K1 1 1, K1 2 0,
                K2 0 0, K2 0 1, K2 1 0, K2 1 1, K2 2 0,
                K3_0 0, K3_0 1, K3_1 0, K4_0 0, K4_0 1, K4_1 0,
                K5_01 0, K5_01 1, K5_12 0, K5_12 1, K6 0, K6 1, K7 0, K7 1,
                hQ2.1, hQ2.2.1, hQ2.2.2, hQ3'.1, hQ3'.2.1, hQ3'.2.2,
                hQ1'.1, hQ1'.2]
    | k + 2 =>
      show Q f (k + 4, 0) - 2 * Q f (k + 3, 0) + Q f (k + 2, 0) ≥ 0
      have hQa := Q0_eq f (k + 2)    -- Q f (k+4,0)
      have hQa' := Q0_le f (k + 2)
      have hQc := Q0_eq f k          -- Q f (k+2,0)
      have hQc' := Q0_le f k
      have hQb := Q0_le f (k + 1)    -- middle Q f (k+3,0): upper bounds only
      rcases hQa with ea | ea | ea <;> rcases hQc with ec | ec | ec <;>
      rw [ea, ec] <;>
      linarith [K1 0 k, K1 0 (k+1), K1 0 (k+2), K1 1 k, K1 1 (k+1), K1 1 (k+2),
                K1 2 k, K1 2 (k+1), K2 0 k, K2 0 (k+1), K2 0 (k+2),
                K2 1 k, K2 1 (k+1), K2 2 k, K2 2 (k+1),
                K3_0 k, K3_0 (k+1), K3_0 (k+2), K3_1 k, K3_1 (k+1),
                K4_0 k, K4_0 (k+1), K4_0 (k+2), K4_1 k, K4_1 (k+1),
                K5_01 k, K5_01 (k+1), K5_01 (k+2), K5_12 k, K5_12 (k+1), K5_12 (k+2),
                K6 k, K6 (k+1), K6 (k+2), K7 k, K7 (k+1),
                hQb.1, hQb.2.1, hQb.2.2,
                hQa'.1, hQa'.2.1, hQa'.2.2, hQc'.1, hQc'.2.1, hQc'.2.2]
  -- ════════════════ y = 1 ════════════════
  · match x with
    | 0 =>
      show Q f (2, 1) - 2 * Q f (1, 1) + Q f (0, 1) ≥ 0
      have hQ2 := Q1_eq f 1          -- Q f (2,1)
      have hQ2' := Q1_le f 1
      have hQ1 := Q1_le f 0          -- middle Q f (1,1): upper bounds
      have hQ0 : Q f (0, 1) = f (0, 1) := by simp only [Q]
      rcases hQ2 with e | e <;> rw [e, hQ0] <;>
      linarith [K1 1 0, K1 1 1, K1 2 0, K2 1 0, K2 2 0,
                K3_1 0, K3_1 1, K4_1 0, K4_1 1, K5_01 0, K5_12 0, K5_12 1,
                K6 0, K6 1, K7 0,
                hQ1.1, hQ1.2, hQ2'.1, hQ2'.2]
    | k + 1 =>
      show Q f (k + 3, 1) - 2 * Q f (k + 2, 1) + Q f (k + 1, 1) ≥ 0
      have hQa := Q1_eq f (k + 2)    -- Q f (k+3,1)
      have hQa' := Q1_le f (k + 2)
      have hQc := Q1_eq f k          -- Q f (k+1,1)
      have hQc' := Q1_le f k
      have hQb := Q1_le f (k + 1)    -- middle Q f (k+2,1): upper bounds
      rcases hQa with ea | ea <;> rcases hQc with ec | ec <;>
      rw [ea, ec] <;>
      linarith [K1 1 k, K1 1 (k+1), K1 1 (k+2), K1 2 k, K1 2 (k+1),
                K2 1 k, K2 1 (k+1), K2 2 k, K2 2 (k+1),
                K3_1 k, K3_1 (k+1), K4_1 k, K4_1 (k+1),
                K5_12 k, K5_12 (k+1), K5_12 (k+2), K6 k, K6 (k+1), K6 (k+2),
                K7 k, K7 (k+1),
                hQb.1, hQb.2, hQa'.1, hQa'.2, hQc'.1, hQc'.2]
  -- ════════════════ y = 2 ════════════════
  · show Q f (x + 2, 2) - 2 * Q f (x + 1, 2) + Q f (x, 2) ≥ 0
    simp only [Q]
    have := K2 2 x
    linarith

/-- (K3) Diagonal increasing differences for `Q f`. Follows from
...
private theorem Q_preserves_K3 (f : ValueFn) (hf : InCone f) :
  ...
  · match x with
    | 0 =>
      ...
      linarith [K1 0 0, K1 0 1, K1 1 0, K3_0 0, K3_1 0, K4_0 0, K4_1 0,
                K5_01 0, K5_12 0, K6 0, K7 0,
    ...
    | k + 1 =>
      ...
      linarith [K1 0 k, K1 0 (k+1), K1 0 (k+2), K1 1 k, K1 1 (k+1), K1 2 k,
                ...
                K4_0 k, K4_0 (k+1), K4_1 k, K4_1 (k+1),
                K5_01 k, K5_01 (k+1), K5_12 k, K5_12 (k+1),
                K6 k, K6 (k+1), K6 (k+2), K7 k, K7 (k+1),
  ...
  · show Q f (x + 2, 1) + Q f (x, 2) - Q f (x + 1, 1) - Q f (x + 1, 2) ≥ 0
    ...
    linarith [K1 1 x, K1 2 x, K3_1 x, K3_1 (x+1), K4_1 x, K4_1 (x+1),
              K5_12 x, K5_12 (x+1), K6 x, K6 (x+1), K7 x,
              hM1.1, hM1.2, hP1'.1, hP1'.2]

/-- (K4) Right multimodularity for `Q f`. Paper pages 9–11. -/
private theorem Q_preserves_K4 (f : ValueFn) (hf : InCone f) :
    ∀ (y : Fin 2) (x : ℕ),
      Q f (x, y.castSucc) + Q f (x + 1, y.succ)
        - Q f (x + 1, y.castSucc) - Q f (x, y.succ) ≥ 0 := by
  intro y x
  -- Cone-fact menu for f (same helpers as K2/K3).
  have K1 : ∀ (z : Fin 3) (a : ℕ), f (a, z) ≤ f (a + 1, z) := by
    intro z a; have := hf.K1 z a; linarith
  have K2 : ∀ (z : Fin 3) (a : ℕ),
      f (a, z) + f (a + 2, z) - 2 * f (a + 1, z) ≥ 0 := by
  ...
  · match x with
    | 0 =>
      ...
      linarith [K1 0 0, K1 1 0, K3_0 0, K4_0 0, K4_1 0,
                K5_01 0, K5_12 0, K6 0, K7 0,
                hM1.1, hM1.2, hP1'.1, hP1'.2]
    | 1 =>
      ...
      linarith [K1 0 0, K1 0 1, K1 1 0, K1 1 1, K1 2 0,
                K2 0 0, K2 1 0, K3_0 0, K3_0 1, K3_1 0, K3_1 1,
                K4_0 0, K4_0 1, K4_1 0, K4_1 1,
                K5_01 0, K5_01 1, K5_12 0, K5_12 1, K6 0, K6 1, K7 0, K7 1,
                hM1.1, hM1.2.1, hM1.2.2, hM2.1, hM2.2,
    ...
    | k + 2 =>
      ...
      linarith [K1 0 k, K1 0 (k+1), K1 0 (k+2), K1 1 k, K1 1 (k+1), K1 1 (k+2),
                ...
                K4_0 k, K4_0 (k+1), K4_0 (k+2), K4_1 k, K4_1 (k+1), K4_1 (k+2),
                K5_01 k, K5_01 (k+1), K5_01 (k+2), K5_12 k, K5_12 (k+1), K5_12 (k+2),
                K6 k, K6 (k+1), K6 (k+2), K7 k, K7 (k+1),
                hM1.1, hM1.2.1, hM1.2.2, hM2.1, hM2.2,
                hP1'.1, hP1'.2.1, hP1'.2.2, hP2'.1, hP2'.2]
  ...
  · match x with
    | 0 =>
      ...
      rw [hP1, hP2, hM2]
      linarith [K1 1 0, K2 2 0, K3_1 0, K4_1 0, K5_12 0, K6 0, K7 0,
                hM1.1, hM1.2]
    | k + 1 =>
      ...
      linarith [K1 1 k, K1 1 (k+1), K1 2 k, K1 2 (k+1),
                K2 1 k, K2 2 k, K3_1 k, K3_1 (k+1), K4_1 k, K4_1 (k+1),
                K5_12 k, K5_12 (k+1), K6 k, K6 (k+1), K7 k,
                hM1.1, hM1.2, hP1'.1, hP1'.2]

/-- (K5) Monotonicity in `y` at fixed `x` for `Q f`. Paper page 11. -/
private theorem Q_preserves_K5 (f : ValueFn) (hf : InCone f) :
    ∀ (y : Fin 2) (x : ℕ), Q f (x, y.succ) - Q f (x, y.castSucc) ≥ 0 := by
  intro y x
  -- The only cone facts K5 needs are (K1) and (K5) of f.
  have K1 : ∀ (z : Fin 3) (a : ℕ), f (a, z) ≤ f (a + 1, z) := by
    intro z a; have := hf.K1 z a; linarith
  have K5_01 : ∀ (a : ℕ), f (a, 0) ≤ f (a, 1) := by
    intro a; have h := hf.K5 0 a
  ...
  · match x with
    ...
    | 1 =>
      ...
      linarith [K1 1 0, K5_01 1, K5_12 0, hM.1, hM.2]
    | k + 2 =>
  ...
  · match x with
    ...
    | k + 1 =>
      ...
      rw [h2]
      linarith [K1 2 k, K5_12 (k + 1), hM.1, hM.2]

/-- (K6) Convexity in `y` at fixed `x` for `Q f`. Paper pages 11–12. -/
private theorem Q_preserves_K6 (f : ValueFn) (hf : InCone f) :
    ∀ x : ℕ, Q f (x, 0) + Q f (x, 2) - 2 * Q f (x, 1) ≥ 0 := by
  intro x
  -- Cone-fact menu for f (same helpers as K2/K3/K4).
  have K1 : ∀ (z : Fin 3) (a : ℕ), f (a, z) ≤ f (a + 1, z) := by
    intro z a; have := hf.K1 z a; linarith
  have K2 : ∀ (z : Fin 3) (a : ℕ),
      f (a, z) + f (a + 2, z) - 2 * f (a + 1, z) ≥ 0 := by
  ...
  | 1 =>
    ...
    linarith [K1 1 0, K4_1 0, K5_01 0, K5_01 1, K5_12 0, K6 0, K6 1,
              hM.1, hM.2]
  | k + 2 =>
    ...
    linarith [K1 0 (k+1), K1 1 (k+1), K1 2 k, K2 1 k, K2 2 k,
              K4_0 (k+1), K4_1 (k+1), K5_01 (k+1), K5_12 k, K5_12 (k+1),
              K6 (k+1), K6 (k+2),
              hM.1, hM.2]

/-- (K7) Convexity in slow-server count at fixed total population for
`Q f`. Follows from `A_ge_B` via `Ā ≥ B̄`. Paper page 7. -/
private theorem Q_preserves_K7 (f : ValueFn) (hf : InCone f) :
